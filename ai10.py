...
# Vector DB 검색
def search_vector_db(query, model, vectorizer):
    if not query:
        return []
    try:
...
def main():
    ...
    with st.spinner("Embedding TRMN_ID and TRMN_NM into ME..."):
        data = query_edi_code()
        if data:
            model, vectorizer = train_vector_db(data)
            st.success("Ready")
        else:
        ...
        if input_text:
            ...
            with st.spinner("Searching for similar diagnoses within ME..."):
                if model and vectorizer:
                    results_indices = search_vector_db(input_text, model, vectorizer)
